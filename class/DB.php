...
class DB {
     ...
     * $Administrateurs = $req->fetchAll(PDO::FETCH_ASSOC); //permet de voir les données en tableau
     * debug($Administrateurs,'brut bdd');
     **/
    ...
    public function save($table, $data, $action){
        ...
        $fields= array();
        $insert= array();
        $insert= array("field"=>array());
        $insert= array("value"=>array());
        $where= array();
        $d= array();
        foreach ($data as $k => $v) {
            $insert['field'][] = $k;
        ...
        if (isset($action['update']) && !empty($action['update']) && is_array($action['update'])) {
            ...
            $sql = "INSERT INTO ".$table."(".implode(', ', $insert['field']).") VALUES(".implode(', ', $insert['value']).")";
            $act = 'insert';
        }
        $pre = $this->db->prepare($sql);
        // debug($pre, '$pre');
